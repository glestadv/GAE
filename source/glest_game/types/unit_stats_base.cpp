...
void UnitStatsBase::addStatic(const EnhancementTypeBase &e, float strength) {
	...
	if (e.getBodyType() != NULL) {			//if non-null, overwrite previous
...
void UnitStatsBase::load(const XmlNode *baseNode, const string &dir, const TechTree *techTree, const FactionType *factionType) {
	...
	//body type string
	const XmlNode *bodyTypeNode = baseNode->getChild("body-type", 0, false);
	if(bodyTypeNode) {
		bodyType = techTree->getBodyType(bodyTypeNode->getAttribute("value")->getRestrictedValue());
	} else {
		bodyType = techTree->getBodyType("unspecified");
	}
...
string &EnhancementTypeBase::getDesc(string &str) const {
	...
	formatModifier(str, "AttackStrenght", attackStrength, attackStrengthMult);
