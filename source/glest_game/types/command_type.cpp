...
const AttackSkillType *AttackSkillTypes::getPreferredAttack(const Unit *unit,
	...
	if(hasPreference(aspWheneverPossible)) {
		...
		assert(ast);
      if(ast->getField(target->getCurrField()==mfAir?fAir:fSurface)) {
			return unit->getMaxRange(ast) >= rangeToTarget ? ast : NULL;
		}
		ast = NULL;
	}
	...
	if(!ast) {
		for(int i = 0; i < types.size(); ++i) {
			if(unit->getMaxRange(types[i]) >= rangeToTarget && types[i]->getField(target->getCurrField()==mfAir?fAir:fSurface)) {
				ast = types[i];
				break;
			}
		}
