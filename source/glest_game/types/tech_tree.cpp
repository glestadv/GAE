...
void TechTree::load(const string &dir, Checksum* checksum){
	...
	try{
		...
		//body types
		const XmlNode *bodyTypesNode= techTreeNode->getChild("body-types", 0, false);
		if(bodyTypesNode) {
			bodyTypes.resize(bodyTypesNode->getChildCount());
			for(int i=0; i<bodyTypes.size(); ++i){
				const XmlNode *bodyTypeNode= bodyTypesNode->getChild("body-type", i);
				bodyTypes[i].setName(bodyTypeNode->getAttribute("name")->getRestrictedValue());
				bodyTypes[i].setId(i);
			}
		} else {
			bodyTypes.resize(1);
			bodyTypes[0].setName("unspecified");
			bodyTypes[0].setId(0);
		}

		//damage multipliers
		damageMultiplierTable.init(attackTypes.size(), armorTypes.size(), bodyTypes.size());
		...
		for(int i=0; i<damageMultipliersNode->getChildCount(); ++i){
			...
			const XmlAttribute *armorAttr = damageMultiplierNode->getAttribute("armor", false);
			const XmlAttribute *bodyAttr = damageMultiplierNode->getAttribute("body", false);
			if(armorAttr && bodyAttr) {
				throw runtime_error("A single damage-multiplier node may not specify both armor and body attributes.");
			} else if(!armorAttr && !bodyAttr) {
				throw runtime_error("damage-multiplier must specify either an armor or body attribute.");
			}
			if(armorAttr) {
				const ArmorType *armorType= getArmorType(armorAttr->getRestrictedValue());
				damageMultiplierTable.setDamageMultiplier(attackType, armorType, multiplier);
			} else {
				const BodyType *bodyType= getBodyType(bodyAttr->getRestrictedValue());
				damageMultiplierTable.setDamageMultiplier(attackType, bodyType, multiplier);
			}
		}
    }
    catch(const exception &e){
...
const BodyType *TechTree::getBodyType(const string &name) const {
	for(int i=0; i<bodyTypes.size(); ++i){
		if(bodyTypes[i].getName()==name){
			return &bodyTypes[i];
		}
	}

	throw runtime_error("Body Type not found: "+name);
}
...
float TechTree::getDamageMultiplier(const AttackType *att, const ArmorType *art, const BodyType *bot) const{
	return damageMultiplierTable.getDamageMultiplier(att, art, bot);
}
