...
CommandResult Commander::pushNetworkCommand(const NetworkCommand* networkCommand) const{
	...
	//validate unit
	if(unit==NULL){
		throw runtime_error("Command refers to non existant unit. Game out of synch. Check that everyone is using the same data.");
	}
...
Command* Commander::buildCommand(const NetworkCommand* networkCommand) const{
	...
	if(unitType!=NULL){
		command= new Command(ct, networkCommand->getFlags(), networkCommand->getPosition(), unitType);
	}
	else if(target==NULL){
		command= new Command(ct, networkCommand->getFlags(), networkCommand->getPosition());
	}
	else{
		command= new Command(ct, networkCommand->getFlags(), target);
	}
