...
public:
	Effect(EffectType* type, Unit *source, Effect *root, float strength,
			const Unit *recipient, const TechTree *tt);
	virtual ~Effect();

	Unit *getSource()				{return source;}
...
private:
	typedef list<Effect*>::const_iterator const_iterator;
	typedef list<Effect*>::iterator iterator;
	list<Effect*> effects;
	bool dirty;
