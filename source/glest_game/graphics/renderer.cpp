...
void Renderer::renderSurface(){
	...
	int currTex;
	const World *world= game->getWorld();
	const int thisTeamIndex= world->getThisTeamIndex();
	...
	while(pqi.next()){
		...
		if(mapBounds.isInside(pos)){
			...
			if(
				tc00->isExplored(thisTeamIndex) ||
				tc10->isExplored(thisTeamIndex) ||
				tc01->isExplored(thisTeamIndex) ||
				tc11->isExplored(thisTeamIndex))
			{
				triangleCount+= 2;
				pointCount+= 4;
...
bool Renderer::computePosition(const Vec2i &screenPos, Vec2i &worldPos){
    ...
    glLoadIdentity();
	gluPickMatrix(screenPos.x, screenPos.y, 1.f, 1.f, view);
	...
	if(glRenderMode(GL_RENDER)>0){
		glRenderMode(GL_SELECT);
		Vec2i pos= Vec2i(selectBuffer[4], selectBuffer[3]);
		renderTile(Vec2i(selectBuffer[4], selectBuffer[3]));

		//compute pos 2
		if(glRenderMode(GL_RENDER)>0){
			worldPos= Vec2i(selectBuffer[4], selectBuffer[3]);
			validPos= true;
		}
		else{
			worldPos= pos*Map::cellScale;
			validPos= true;
		}
	}
	glMatrixMode(GL_PROJECTION);
...
//this fc renders surface for selection
void Renderer::renderSurfaceFast(){
	...
	while(pqi.next()){
		...
		if(mapBounds.isInside(pos)){
			...
			glPushName(pos.x);
			glBegin(GL_TRIANGLE_FAN);
             	glVertex3fv(map->getSurfaceCell(pos.x, pos.y+1)->getVertex().ptr());
             	glVertex3fv(map->getSurfaceCell(pos.x, pos.y)->getVertex().ptr());
              	glVertex3fv(map->getSurfaceCell(pos.x+1 ,pos.y)->getVertex().ptr());
              	glVertex3fv(map->getSurfaceCell(pos.x+1, pos.y+1)->getVertex().ptr());
			glEnd();
			glPopName();
			glPopName();
		}
