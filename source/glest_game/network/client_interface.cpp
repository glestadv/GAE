	...
	switch(networkMessageType){
		...
		case nmtIntro:{
			...
			if(receiveMessage(&networkMessageIntro)){
				...
				//check consistency
				if(Config::getInstance().getNetworkConsistencyChecks() && networkMessageIntro.getVersionString() != getNetworkVersionString()){
					throw runtime_error("Server and client versions do not match ("
							+ networkMessageIntro.getVersionString() + "). You have to use the same binaries.");
				}
