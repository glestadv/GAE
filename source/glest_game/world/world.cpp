...
void World::init(Game *game, const XmlNode *worldNode) {
	...
	if(worldNode) {
		map.load(worldNode->getChild("map"), this);
	}
	computeFow();
	alive = true;
}
...
inline void World::moveAndEvict(Unit *unit, vector<Unit*> &evicted, Vec2i *oldPos) {
	...
	if(!oldPos || *oldPos != pos) {
		...
		for(vector<Unit*>::const_iterator i = evicted.begin(); i != evicted.end(); ++i) {
			isEvicted = true;
			break;
		}
		if(oldPos && !isEvicted) {
...
void logUnit(Unit *unit, string action, Vec2i *oldPos, int *oldHp) {
	Logger::getInstance().add(action + " " + unit->getType()->getName()
...
void World::doClientUnitUpdate(XmlNode *n, bool minor, vector<Unit*> &evicted) {
	Unit *unit = UnitReference(n).getUnit();
	assert(unit);
	...
	if(Config::getInstance().getDebugMode()) {
		logUnit(unit, "update", &lastPos, &lastHp);
	}
	...
	if(lastHp || lastSkill->getClass() != scDie) {
		// are they alive now?
		if(unit->getHp()) {
			moveAndEvict(unit, evicted, wasEvicted ? NULL : &lastPos);
		} else {
			unit->kill(lastPos, !wasEvicted);
		}
	} else {
...
void World::update(){
	...
	if(networkManager.isNetworkGame() && networkManager.isServer()) {
		networkManager.getServerInterface()->sendUpdates();
	}
}
