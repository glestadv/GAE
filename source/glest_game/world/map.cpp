...
Earthquake::Earthquake(Map &map, Unit *cause, const Vec2i &uepicenter, const EarthquakeType* type, float strength) :
	...
	for(int x = bounds.p[0].x; x <= bounds.p[1].x; ++x) {
		for(int y = bounds.p[0].y; y <= bounds.p[1].y; ++y) {
			*height++ = map.getSurfaceCell(x, y)->getHeight();
		}
	}
	assert(height == &heights[cellCount]);
}
...
inline float Earthquake::temporalPressure(float time) {
	if(time < -initialRadius / speed || time > duration) {
		return 0.f;
	}

	return sin(time * frequency * 2.f * M_PI) * temporalMagnitude(time / duration) * magnitude;
}
...
void Earthquake::update(float slice) {
	...
	for(int x = bounds.p[0].x; x <= bounds.p[1].x; ++x) {
		for(int y = bounds.p[0].y; y <= bounds.p[1].y; ++y) {
			...
			float pressure = temporalPressure(time - timeLag) * decayMultiplier;

			map.getSurfaceCell(pos)->setHeight(*height++ + pressure);
		}
	}
...
void Earthquake::getDamageReport(std::map<Unit *, float> &results, float slice) {
