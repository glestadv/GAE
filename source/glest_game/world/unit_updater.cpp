...
void UnitUpdater::updateUnitCommand(Unit *unit){
	...
	//if unit is set to wait for server, we won't update command
	if(unit->getCurrSkill()->getClass() == scWaitForServer) {
		return;
	}
...
void UnitUpdater::updateBuild(Unit *unit){
	...
	// make sure we're talking about the same build command
	if(unit->getCurrSkill()->getClass() != scBuild && (!target
		  || target->getPos() != command->getPos() || target->getType() != builtUnitType)) {
		...
		if(inside && !command->hasWaypoint()) {
			// need to find a place to go to build.

			Vec2i waypoint;
			if(map->getNearestAdjacentFreePos(waypoint, unit, pos, fLand, buildingSize)) {
				command->setWaypoint(waypoint);
			} else {
		...
		pos = command->hasWaypoint() ? command->getWaypoint()
			: Map::getNearestPos(unit->getPos(), command->getPos(), 1, 1, builtUnitType->getSize());
		if(pos != unit->getTargetPos()) {
			unit->setTargetPos(pos);
			unit->getPath()->clear();
		}
		...
		case PathFinder::tsArrived: {
			...
			Vec2i nearPos = Map::getNearestPos(unit->getPos(), command->getPos(), 1, 1, builtUnitType->getSize());

			if(unit->getPos() != nearPos) {
				if(command->hasWaypoint()) {
					command->setWaypoint(command->getPos() + Vec2i(builtUnitType->getSize() / 2));
				} else {
					command->setWaypoint(Map::getNearestPos(unit->getPos(), command->getPos(), 0, 0, builtUnitType->getSize()));
				}
				unit->setTargetPos(command->getWaypoint());
				unit->getPath()->clear();
				break;
			}
			...
			if(map->isFreeCells(command->getPos(), ut->getSize(), fLand)) {
				...
				Unit *builtUnit= map->getCell(command->getPos())->getUnit(fLand);
...
void UnitUpdater::updateRepair(Unit *unit){
	...
	if(repairableOnRange(unit, &repaired, rct, rst->getMaxRange(), rst->isSelfAllowed())) {
		...
		if(repairableOnSight(unit, &repaired, rct, rst->isSelfAllowed())) {
			targetPos = command->getWaypoint().x != -1 ? command->getWaypoint()
				: Map::getNearestPos(unit->getPos(), repaired, 1, rst->getMaxRange());
		...
		case PathFinder::tsArrived:
			if(repaired && unit->getPos() != targetPos) {
				if(command->getWaypoint().x == -1) {
					command->setWaypoint(Map::getNearestPos(unit->getPos(), repaired, 0, rst->getMaxRange() - 1));
				} else {
					command->setWaypoint(repaired->getCenteredPos());
				}
				unit->setTargetPos(command->getWaypoint());
				unit->getPath()->clear();
				break;
	...
	if(repairThisFrame && unit->getCurrSkill()->getClass() == scRepair) {
		...
		if(!repaired) {
			...
			/*
			if(isNetworkGame() && isServer()) {
				getServerInterface()->minorUnitUpdate(repaired);
			}*/
		}
	}
}
