	...
	//we can repair any ally => repair it
   	if(unit->getType()->hasSkillClass(scAttack) || unit->getType()->hasSkillClass(scRepair)) {
		...
		for(int i=0; i<unit->getType()->getCommandTypeCount(); ++i){
			...
			switch(ct->getClass()) {
				case ccAttack:
					ast= ((const AttackCommandType*)ct)->getAttackSkillType();
					break;
				case ccAttackStopped:
					ast= ((const AttackStoppedCommandType*)ct)->getAttackSkillType();
					break;
				case ccRepair:
			...
			if(ast) {
				if(attackableOnSight(unit, &sighted, ast)){
					unit->giveCommand(new Command(ct, CommandFlags(cpAuto), sighted->getPos()));
					unit->giveCommand(new Command(ct, CommandFlags(cpQueue, cpAuto, cpAutoReturn), unit->getPos()));
					return;
				}
			}
			...
			if(rct) {
				if(repairableOnSight(unit, &sighted, rct)) {
					const RepairSkillType *rst = rct->getRepairSkillType();
					unit->giveCommand(new Command(rct, CommandFlags(cpQueue, cpAuto), getNear(unit->getPos(), sighted, 1, rst->getMaxRange())));
					unit->giveCommand(new Command(rct, CommandFlags(cpQueue, cpAuto, cpAutoReturn), unit->getPos()));
				}
			}
		}
	}
...
void UnitUpdater::updateMove(Unit *unit){
	...
	Vec2i pos = command->getUnit()
			? unit->getCenteredPos() //getNear(unit->getPos(), command->getUnit(), 1, FOLLOW_DISTANCE)
			: command->getPos();
...
bool UnitUpdater::updateAttackGeneric(Unit *unit, const Command *command, const AttackCommandType *act, Unit* target, const Vec2i &targetPos){
    ...
    if(attackableOnRange(unit, &target, act->getAttackSkillType())){
		...
		//compute target pos
		Vec2i pos;
		if(attackableOnSight(unit, &target, act->getAttackSkillType())){
			pos= target->getCenteredPos();
		} else {
			//if auto and no more targets then finish
			if(command->isAuto()) {
				return true;
			}
			pos= targetPos;
	...
	if(unit->getCurrSkill()->getClass() != scBuild) {
		...
		if(inside && command->getPos2().x == 0) {
			...
			for(i = 0; i < 50; i++) {
				...
				if(map->isFreeCells(newpos, unit->getSize(), fLand)) {
		...
		} else if(!inside && command->getPos2().x != 0) {
			command->setPos2(Vec2i(0));
		}

		pos = inside ? command->getPos2()
			: getNear(unit->getPos(), command->getPos(), builtUnitType->getSize(), 1, 1);
...
void UnitUpdater::updateRepair(Unit *unit){
	...
	if(repairableOnRange(unit, &repaired, rct, rst->getMaxRange())) {
		unit->setTarget(repaired);
		unit->setCurrSkill(rst);
	} else {
		Vec2i targetPos;
		if(repairableOnRange(unit, &repaired, rct, unit->getSight())) {
...
void UnitUpdater::damage(Unit *attacker, const AttackSkillType* ast, Unit *attacked, float distance){
	...
	float damageMultiplier= world->getTechTree()->getDamageMultiplier(ast->getAttackType(),
			attacked->getType()->getArmorType(), attacked->getType()->getBodyType());
	...
	//complain
	if(gui->isVisible(attacked->getCurrVector())) {
 ...
 * the target, but no greater than maxRange.
 */
Vec2i UnitUpdater::getNear(const Vec2i &orig, const Vec2i dest, int minRange, int maxRange) {
		Vec2f forig;
		Vec2f fdest;
		Vec2f fresult;
		Vec2i iresult;
		...
		forig.x = (float)orig.x;
		forig.y = (float)orig.y;
		fdest.x = (float)dest.x;
		fdest.y = (float)dest.y;
		float len = forig.dist(fdest);

	if(len > maxRange) {
		if(len >= minRange) {
			fresult = fdest.lerp((float)maxRange / len, forig);
			iresult.x = (int)(fresult.x + 0.5);
			iresult.y = (int)(fresult.y + 0.5);
			return iresult;
		} else {
			fresult = fdest.lerp((float)minRange / len, forig);
			iresult.x = (int)(fresult.x + 0.5);
			iresult.y = (int)(fresult.y + 0.5);
			return iresult;
		}
...
bool UnitUpdater::attackableOnRange(const Unit *unit, Unit **rangedPtr, const AttackSkillType *ast){
	int range= unit->getMaxRange(ast);
	return unitOnRange(unit, range, rangedPtr, ast);
}
...
bool UnitUpdater::repairableOnRange(
	...
	//nearby cells
	for (int i = center.x - range; i < center.x + range + centerSize - 1; ++i){
		for (int j = center.y - range; j < center.y + range + centerSize - 1; ++j){
