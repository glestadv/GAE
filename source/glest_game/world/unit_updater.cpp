...
//					}
	...
	if(unit->getCommands().size() == 1 && unit->getType()->hasCommandClass(ccMove)
		...
		if(unit->getType()->hasCommandClass(ccRepair)) {
			...
			Unit *myHero = NULL;
			int range = (myCenter.dist(signtedCenter) - (unit->getType()->getSize() + sighted->getType()->getSize())) / 1.75f;
...
void UnitUpdater::updateBuild(Unit *unit){
	...
	if(unit->getCurrSkill()->getClass() != scBuild) {
		...
		if(inside && command->getWaypoint().x == -1) {
			...
			command->setWaypoint(newpos);
		}// else if(!inside && command->getWaypoint().x != -1) {
		//	command->setWaypoint(Vec2i(-1));
		//}

		pos = command->getWaypoint().x != -1 ? command->getWaypoint()
		...
		case PathFinder::tsArrived: {
			...
			Vec2i nearPos = getNear(unit->getPos(), command->getPos(), 1, 1, builtUnitType->getSize());

			if(unit->getPos() != nearPos) {
				if(command->getWaypoint().x == -1) {
					command->setWaypoint(getNear(unit->getPos(), command->getPos(), 0, 0, builtUnitType->getSize()));
				} else {
	...
	if(repairableOnRange(unit, &repaired, rct, rst->getMaxRange(), rst->isSelfAllowed())) {
		...
		if(repairableOnSight(unit, &repaired, rct, rst->isSelfAllowed())) {
			targetPos = command->getWaypoint().x != -1 ? command->getWaypoint()
				: getNear(unit->getPos(), repaired, 1, rst->getMaxRange());
		...
		case PathFinder::tsArrived:
			if(repaired && unit->getPos() != targetPos) {
				if(command->getWaypoint().x == -1) {
					command->setWaypoint(getNear(unit->getPos(), repaired, 0, rst->getMaxRange() - 1));
				} else {
