...
public:
	...
	void setHeight(float height)					{originalHeight = vertex.y= height;}
	...
	void resetHeight()								{vertex.y = originalHeight;}
	void alterHeight(float offset)					{vertex.y += offset;}
	void updateObjectHeight() {
		if(object) {
			object->setHeight(vertex.y);
		}
	}
...
private:
	...
	float temporalPressure(float time);
