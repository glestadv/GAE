...
private:
	...
	bool repairableOnSight(const Unit *unit, Unit **rangedPtr, const RepairCommandType *rct) {
		return repairableOnRange(unit, rangedPtr, rct, unit->getSight());
	}

	void enemiesAtDistance(const Unit *unit, const Unit *priorityUnit, int distance, vector<Unit*> &enemies);
	bool updateAttackGeneric(Unit *unit, const Command *command, const AttackCommandType *act, Unit* target, const Vec2i &targetPos);
	Vec2i getNear(const Vec2i &orig, Vec2i dest, int minRange, int maxRange);
	Vec2i getNear(const Vec2i &orig, Vec2i dest, int destSize, int minRange, int maxRange);
	Vec2i getNear(const Vec2i &orig, const Unit *destUnit, int minRange, int maxRange) {
		int size = destUnit->getType()->getSize();
		Vec2i pos = destUnit->getPos();
		if(size == 1) {
			return getNear(orig, pos, minRange, maxRange);
		} else {
			return getNear(orig, pos, size, minRange, maxRange);
		}
	}
};
