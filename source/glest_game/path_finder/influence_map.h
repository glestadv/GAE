...
public:
	InfluenceMap(Rect b, iType def) : x(b.x), y(b.y), w(b.w), h(b.h), def(def) {}
...
public:
	TypeMap(Rect b, type def) : InfluenceMap<type, TypeMap<type>>(b,def) {
		data = new type[w*h];
	}
...
public:
	PatchMap(Rect b, uint32 def) : InfluenceMap<uint32,PatchMap<bits> >(b,def) {
...
public:
	FlowMap(Rect b, Point def) : InfluenceMap<Point,FlowMap>(b,def) {
