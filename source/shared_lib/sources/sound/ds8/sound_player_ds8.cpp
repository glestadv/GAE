...
void SoundBuffer::createDsBuffer(IDirectSound8 *dsObject) {
	...
	format.nAvgBytesPerSec = (sound->getBitsPerSample() * sound->getSamplesPerSecond() * sound->getChannels()) / 8;
	format.nBlockAlign = (sound->getChannels() * sound->getBitsPerSample()) / 8;
	format.wBitsPerSample = sound->getBitsPerSample();
	format.cbSize = 0;
...
void SoundPlayerDs8::play(StaticSound *staticSound, float attenuation) {
	int bufferIndex = -1;
...
long dsVolume(float floatVolume) {
	...
	return clamp<long>(vol, DSBVOLUME_MIN, DSBVOLUME_MAX);
}
