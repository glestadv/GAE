...
ALenum SoundSource::getFormat(const Sound &sound) {
	if (sound.getChannels() == 2) {
		if (sound.getBitsPerSample() == 16) {
			return AL_FORMAT_STEREO16;
		} else if (sound.getBitsPerSample() == 8) {
			return AL_FORMAT_STEREO8;
		} else {
			throw std::runtime_error("Sample format not supported");
		}
	} else if (sound.getChannels() == 1) {
		if (sound.getBitsPerSample() == 16) {
...
void StaticSoundSource::play(const StaticSound &sound, float volume) {
	...
	alBufferData(buffer, format, sound.getSamples(),
	             static_cast<ALsizei> (sound.getSize()),
