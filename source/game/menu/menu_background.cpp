...
MenuBackground::MenuBackground(const Config &config) : config(config) {
    ...
    if (rain) {
        ...
        rps->setSpeed(12.f / getConfig().getGsWorldUpdateFps());
        rps->setEmissionRate(25);
        rps->setWindSpeed2(-90.f, 4.f / getConfig().getGsWorldUpdateFps());
...
void MenuBackground::update() {
    ...
    //rain drops
    for (int i = 0; i < raindropCount; ++i) {
        raindropStates[i] += 1.f / getConfig().getGsWorldUpdateFps();
    ...
    if (targetCamera != NULL) {
        t += ((0.01f + (1.f - t) / 10.f) / 20.f) * (60.f / getConfig().getGsWorldUpdateFps());
    ...
    //fade
    if (fade <= 1.f) {
        fade += 0.6f / getConfig().getGsWorldUpdateFps();
        if (fade > 1.f) {
            fade = 1.f;
        }
    }
    ...
    //animation
    anim += (0.6f / getConfig().getGsWorldUpdateFps()) / 5 + random.randRange(0.f, (0.6f / getConfig().getGsWorldUpdateFps()) / 5.f);
    if (anim > 1.f) {
        anim = 0.f;
    }
}
