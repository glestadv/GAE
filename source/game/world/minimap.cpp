...
Minimap::Minimap(Renderer &renderer, bool fogOfWar)
		: renderer(renderer)
		, fogOfWar(fogOfWar)
		, fowPixmap0(NULL)
		, fowPixmap1(NULL)
...
/**
 * Read state from buf. See @{see Minimap::write(NetworkDataBuffer &buf) const} for description.
 */
...
/**
 * Writes the minimap state to the NetworkDataBuffer.  Implementation Note: the data produced by
 * this function may be more compressable if we remove our own cheap compression by writing
 * repitition, but this is far more CPU-friendly in any event and works well enough for now.
 *
...
/**
 * Synthesize alphas based upon explored areas from map object (used when deserializing a minimap).
 * Theory: In order to proximate the proper alphas from when areas are explored in normal game play,
 * the exploration states of the surrounding 120 cells are examined.  If less than 100 of those
 * cells are explored, the alpha for the center cell is completely black, otherwise, an alpha
 * between 0.25 and 0.5 is used, 0.25 if only 110 of the neighbors are explored and 0.5 if all 120
 * of them are explored.
 *
 * Alpha states from the cell being visible will be calculated later, so visibility is ignored here.
 */
