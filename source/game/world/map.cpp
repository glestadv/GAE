		...
		for (int y = 0; y < m_tileSize.h; ++y) {
			for (int x = 0; x < m_tileSize.w; ++x) {
				...
				if (objNumber == 0 || x == 0 || y == 0 || x >= m_tileSize.w - 2 || y >= m_tileSize.h - 2) {
					...
					MapObject *o = g_world.newMapObject(tileset->getObjectType(objNumber - 1), Vec2i(x, y), vert, tileset->getObjectSeed());
					tile->setObject(o);
					...
					MapObject *o = g_world.newMapObject(NULL, Vec2i(x, y), vert, tileset->getObjectSeed());
					tile->setObject(o);
					o->setResource(rt, Vec2i(x, y) * cellScale);
				}
			}
