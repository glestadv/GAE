...
//load tileset
bool World::loadTileset(Checksums &checksums) {
...
//load tech
bool World::loadTech(Checksums &checksums) {
	set<string> names;

	const GameSettings::Factions &gsFactions = gs.getFactions();
	for (int i = 0; i < gsFactions.size(); ++i) {
		names.insert(gsFactions[i]->getTypeName());
	}

	return techTree.load(gs.getTechPath(), names, checksums);
}

//load map
bool World::loadMap(Checksums &checksums) {
...
static void logUnit(Unit *unit, string action, Vec2i *oldPos, int *oldHp) {
	Logger::getClientLog().add(action + " " + unit->getType()->getName()
			...
			+ (oldPos ? pos2str(*oldPos) : string("none")) + " => " + pos2str(unit->getPos())
