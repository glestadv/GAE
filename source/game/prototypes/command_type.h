...
/** interface CmdDesriptor FIXME: send to Glest::Gui */
class CmdDescriptor {
public:
...
protected:
	...
	void setUnitType( const UnitType *ut ) { m_unitType = ut; }
...
public:
	...
	void describe( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt = nullptr ) const;
	virtual void descSkills( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt = nullptr ) const = 0;
	...
	virtual string getTipKey( const string &name ) const  {return m_emptyString;}
	...
	/** return the thing (ProducibleType derivitive) this command makes at index i.
	  * CommandTypes that make stuff MUST override this method */
	virtual ProdTypePtr getProduced( int i ) const    { return nullptr; }
	...
	bool isQueuable( ) const                     { return m_queuable; }
	...
	virtual Clicks        getClicks( ) const     { return m_clicks; }
	string getDesc( const Unit *unit ) const     {
		string str;
		//str = name + "\n";
		getDesc( str, unit );
		if (m_energyCost) {
			str += "\n" + g_lang.get( "EnergyCost" ) + ": " + intToStr( m_energyCost );
		}
		return str;
	}
...
public:
	MoveBaseCommandType( const char* name, Clicks clicks ) : CommandType( name, clicks ), m_moveSkillType( nullptr ) {}
	virtual void doChecksum( Checksum &checksum ) const {
...
public:
	StopBaseCommandType( const char* name, Clicks clicks ) : CommandType( name, clicks ), m_stopSkillType( nullptr ) { }
	virtual void doChecksum( Checksum &checksum ) const {
		CommandType::doChecksum( checksum );
...
public:
	...
	virtual void descSkills( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt = nullptr ) const override { }
...
// ===============================
// this is not derived from any other CommandType to avoid diamonds (is multiply inherited)

class AttackCommandTypeBase {
protected:
	AttackSkillTypes m_attackSkillTypes;

public:
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft, const UnitType *ut );
	virtual void getDesc( string &str, const Unit *unit ) const { m_attackSkillTypes.getDesc( str, unit ); }
	virtual void doChecksum( Checksum &checksum ) const {
		m_attackSkillTypes.doChecksum( checksum );
	}
...
// const AttackSkillType *getAttackSkillType(Field field) const;
	const AttackSkillTypes *getAttackSkillTypes( ) const	{ return &m_attackSkillTypes; }
};
...
public:
	BuildCommandType( ) : MoveBaseCommandType( "Build", Clicks::TWO ), m_buildSkillType( nullptr ) { }
	~BuildCommandType( );
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
...
public:
	HarvestCommandType( ) : MoveBaseCommandType( "Harvest", Clicks::TWO )
		, m_moveLoadedSkillType( nullptr ), m_harvestSkillType( nullptr ), m_stopLoadedSkillType( nullptr )
...
private:
	const RepairSkillType*   m_repairSkillType;
	vector<const UnitType*>  m_repairableUnits;

public:
	RepairCommandType( ) : MoveBaseCommandType( "Repair", Clicks::TWO ), m_repairSkillType( nullptr ) { }
	~RepairCommandType( ) { }
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
	virtual void getDesc( string &str, const Unit *unit ) const;
	...
	//get
	const RepairSkillType *getRepairSkillType( ) const	{ return m_repairSkillType; }
...
public:
	ProduceCommandType( ) : CommandType( "Produce", Clicks::ONE, true ), m_produceSkillType( nullptr ) { }
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
	virtual void getDesc( string &str, const Unit *unit ) const;
	virtual void subDesc( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const override;
...
public:
	GenerateCommandType( ) : CommandType( "Generate", Clicks::ONE, true ), m_produceSkillType( nullptr ) { }
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
	virtual void getDesc( string &str, const Unit *unit ) const;
	virtual void subDesc( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const override;
...
public:
	UpgradeCommandType( ) : CommandType( "Upgrade", Clicks::ONE, true ), m_upgradeSkillType( nullptr ) { }
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
	virtual string getReqDesc( const Faction *f ) const;
...
public:
	LoadCommandType( ) 
			...
			, loadSkillType( nullptr ), moveSkillType( nullptr ), m_loadCapacity( 0 ), m_allowProjectiles( false ) {
		m_queuable = true;
	}
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
...
private:
	const MoveSkillType *m_moveSkillType;
	const UnloadSkillType *m_unloadSkillType;

public:
	UnloadCommandType( ) : CommandType( "Unload", Clicks::TWO ), m_unloadSkillType( nullptr ), m_moveSkillType( nullptr ) { }
	virtual bool load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft );
	virtual void doChecksum( Checksum &checksum ) const;
	virtual void getDesc( string &str, const Unit *unit ) const;
	virtual void descSkills( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt = nullptr ) const override;
	...
	//get
	const MoveSkillType *getMoveSkillType( ) const      { return m_moveSkillType;   }
	const UnloadSkillType *getUnloadSkillType( ) const  { return m_unloadSkillType; }

	virtual Clicks getClicks( ) const { return (m_moveSkillType ? Clicks::TWO : Clicks::ONE); }
	virtual CmdClass getClass( ) const { return typeClass( ); }
...
public:
	BeLoadedCommandType( )
			: CommandType( "BeLoaded", Clicks::ONE ), m_moveSkillType( nullptr ) { }

	void setMoveSkill( const MoveSkillType *moveSkill ) { m_moveSkillType = moveSkill; }
...
public:
	CastSpellCommandType( ) : CommandType( "CastSpell", Clicks::ONE ), m_cycle( false ), m_castSpellSkillType( nullptr ) {}
	virtual void doChecksum( Checksum &checksum ) const {
		CommandType::doChecksum( checksum );
...
public:
	BuildSelfCommandType( ) : CommandType( "build-self", Clicks::TWO ), m_buildSelfSkill( nullptr ) { }
...
public:
	Targets() : nearest( nullptr ), distance( fixed::max_value( ) ) { }
