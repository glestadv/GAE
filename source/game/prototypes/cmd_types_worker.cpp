...
/** rangedPtr should point to a pointer that is either nullptr or a valid Unit */
bool RepairCommandType::repairableInRange( const Unit *unit, Vec2i centre, int centreSize,
	...
	while (pci.getNext( pos, distance )) {
		...
		for (int z = 0; z < Zone::COUNT; z++) {
			Unit *candidate = map->getCell( pos )->getUnit( Field( z ) );
			if (candidate == nullptr) {
				continue;
			}
			const Command *cmd = candidate->anyCommand( ) ? candidate->getCurrCommand( ) : nullptr;
			...
			const BuildSelfCommandType *bsct = cc == CmdClass::BUILD_SELF 
				? static_cast<const BuildSelfCommandType*>( cmd->getType( ) ) : nullptr;
...
bool RepairCommandType::load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft ) {
	...
	try {
		...
		m_repairSkillType= static_cast<const RepairSkillType*>( m_unitType->getSkillType( skillName, SkillClass::REPAIR ) );
	...
	try {
		...
		for (int i=0; i<unitsNode->getChildCount( ); ++i) {
			...
			m_repairableUnits.push_back( ft->getUnitType( unitNode->getAttribute( "name" )->getRestrictedValue( ) ) );
		}
	} catch (runtime_error e) {
		g_logger.logXmlError( dir, e.what( ) );
		loadOk = false;
...
void RepairCommandType::doChecksum( Checksum &checksum ) const {
	...
	checksum.add( m_repairSkillType->getName( ) );
	for (int i=0; i < m_repairableUnits.size( ); ++i) {
		checksum.add( m_repairableUnits[i]->getName( ) );
	}
}
...
void RepairCommandType::getDesc( string &str, const Unit *unit ) const{
	...
	m_repairSkillType->getDesc( str, unit );

	str+="\n" + lang.get( "CanRepair" ) + ":\n";
	if (m_repairSkillType->isSelfOnly( )) {
		str += lang.get( "SelfOnly" );
	} else if(m_repairSkillType->isPetOnly( )) {
		str += lang.get( "PetOnly" );
	} else {
		for (int i=0; i < m_repairableUnits.size( ); ++i) {
			const UnitType *ut = (const UnitType*)m_repairableUnits[i];
...
void RepairCommandType::descSkills( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const {
	string msg;
	m_repairSkillType->getDesc( msg, unit );
...
void RepairCommandType::subDesc( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const {
	...
	callback->addElement( lang.get( "CanRepair" ) + ":" );
	if (m_repairSkillType->isSelfOnly( )) {
		callback->addElement( lang.get( "SelfOnly" ) );
	} else if (m_repairSkillType->isPetOnly( )) {
		callback->addElement( lang.get( "PetOnly" ) );
	} else {
		foreach_const (vector<const UnitType*>, it, m_repairableUnits) {
			string name;
...
bool RepairCommandType::canRepair( const UnitType *unitType ) const{
	for(int i=0; i < m_repairableUnits.size( ); ++i) {
		if (static_cast<const UnitType*>( m_repairableUnits[i] ) == unitType) {
			return true;
		}
	}
	return false;
...
void RepairCommandType::update( Unit *unit ) const {
	...
	const RepairSkillType* const &rst = m_repairSkillType;
	...
	if (repaired && (repaired->isDead( ) || !repaired->isDamaged( ) || !unit->isAlly( repaired ))) {
				...
				if (!wasBuilt) {
					...
					if (unit->getFactionIndex( ) == g_world.getThisFactionIndex( )) {
						...
						if (bct) {
							...
							g_soundRenderer.playFx( bct->getBuiltSound( ), unit->getCurrVector( ), g_gameState.getGameCamera( )->getPos( ) );
						}
					}
				}
			}
...
Command *RepairCommandType::doAutoRepair( Unit *unit ) const {
	...
	if (unit->getEp( ) >= m_repairSkillType->getEpCost( )
	&& repairableInSight( unit, &sighted, this, m_repairSkillType->isSelfAllowed( ) )) {
		REPAIR_LOG( unit, __FUNCTION__ << "( ): Unit:" << *unit << " @ " << unit->getPos( )
			<< ", found someone ( " << *sighted << " ) to repair @ " << sighted->getPos( ) );
		Command *newCommand;
		
		Vec2i pos = Map::getNearestPos( unit->getPos( ), sighted, m_repairSkillType->getMinRange( ), m_repairSkillType->getMaxRange( ) );
...
bool BuildCommandType::load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft ) {
	...
	try {
		...
		m_buildSkillType = static_cast<const BuildSkillType*>( m_unitType->getSkillType( skillName, SkillClass::BUILD ) );
	} catch (runtime_error e) {
		g_logger.logXmlError( dir, e.what( ) );
		loadOk = false;
	}
...
void BuildCommandType::subDesc( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const {
	if (!pt) {
		...
		const string factionName = unit->getFaction( )->getType( )->getName( );
		// buildings built
		callback->addElement( lang.get( "Buildings" ) + ":" );
...
void BuildCommandType::update( Unit *unit ) const {
	...
	if (unit->getCurrSkill( )->getClass( ) != SkillClass::BUILD) {
		...
		if (map->canOccupy( command->getPos( ), builtUnitType->getField( ), builtUnitType, command->getFacing( ) )) {
			...
			Unit *builtUnit = occupants.size( ) == 1 ? occupants[0] : nullptr;
...
bool HarvestCommandType::load( const XmlNode *n, const string &dir, const TechTree *tt, const FactionType *ft ) {
	...
	try {
		...
		m_harvestSkillType = static_cast<const HarvestSkillType*>( m_unitType->getSkillType( skillName, SkillClass::HARVEST ) );
	} catch (runtime_error e) {
		g_logger.logXmlError( dir, e.what( ) );
		loadOk = false;
	}
	...
	try { 
		...
		m_stopLoadedSkillType = static_cast<const StopSkillType*>( m_unitType->getSkillType( skillName, SkillClass::STOP ) );
	} catch (runtime_error e) {
		g_logger.logXmlError( dir, e.what( ) );
		loadOk = false;
	}
	...
	try {
		...
		m_moveLoadedSkillType = static_cast<const MoveSkillType*>( m_unitType->getSkillType( skillName, SkillClass::MOVE ) );
	} catch (runtime_error e) {
		g_logger.logXmlError( dir, e.what( ) );
		loadOk = false;
	}
...
void HarvestCommandType::subDesc( const Unit *unit, CmdDescriptor *callback, ProdTypePtr pt ) const {
	Lang &lang = g_lang;
	callback->addElement( lang.get( "Harvested" ) + ":" );
...
const MoveSkillType* getMoveLoadedSkill( Unit *u ) {
	const MoveSkillType *mst = nullptr;
...
const StopSkillType* getStopLoadedSkill( Unit *u ) {
	const StopSkillType *sst = nullptr;
	for (int i=0; i < u->getType( )->getCommandTypeCount<HarvestCommandType>( ); ++i) {
		const HarvestCommandType *t = u->getType( )->getCommandType<HarvestCommandType>( i );
		if (t->canHarvest( u->getLoadType( ) )) {
...
/// nullptr if no resource was found within maxResSearchRadius.
