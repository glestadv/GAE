	...
	switch (unit->getCurrSkill()->getClass()) {
		case SkillClass::MOVE:
			...
			SYNC_LOG( "Cmd:: Frame: " << frame << ", Unit: " << unit->getId() << ", FactionIndex: " << unit->getFactionIndex() 
				<< ", CurrCmdId: " << unit->getCurrCommand()->getType()->getId() << ", CurrSkillId: " << unit->getCurrSkill()->getId() << ", NextPos: " << unit->getNextPos() );
			break;
		...
		case SkillClass::BUILD:
			...
			SYNC_LOG( "Cmd:: Frame: " << frame << ", Unit: " << unit->getId() << ", FactionIndex: " << unit->getFactionIndex() 
				<< ", CurrCmdId: " << unit->getCurrCommand()->getType()->getId() << ", CurrSkillId: " << unit->getCurrSkill()->getId() << ", TargetUnit: " << unit->getTarget() );
			break;
		default:
			SYNC_LOG( "Cmd:: Frame: " << frame << ", Unit: " << unit->getId() << ", FactionIndex: " << unit->getFactionIndex()
				<< ", CurrCmdId: " << (unit->anyCommand() ? unit->getCurrCommand()->getType()->getId() : -1) << ", CurrSkillId: " << unit->getCurrSkill()->getId() );
			break;
	}
